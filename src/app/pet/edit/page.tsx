...
export default function EditPet() {
  ...
  useEffect(() => {
    if (!loading && !isAuthenticated) {
      alert("You need to be logged in to edit a pet.");
      router.push("/login");
      return;
    }
    ...
    const fetchPetData = async () => {
      try {
        ...
        alert(`Error fetching pet data: ${error.response?.data?.message || error.message}`);
      } finally {
        setIsLoading(false);
      }
    };

    if (isAuthenticated) fetchPetData();
  }, [isAuthenticated, loading, petId, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        alert(`Failed to update pet: ${response.data.message}`);
      }
    } catch (error: any) {
      alert(`Error updating pet: ${error.response?.data?.message || error.message}`);
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 p-4">
      <h1 className="text-2xl font-bold text-center text-gray-800 mb-6">Edit Pet</h1>
